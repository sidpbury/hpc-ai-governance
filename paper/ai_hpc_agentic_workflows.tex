...
A Layered Architecture for Secure and Human-Governed Research Computing}
...
\begin{abstract}
This paper examines the use of large language model (LLM) agents in a production-oriented high-performance computing (HPC) setting and evaluates a progressive governance approach for moving from advisory assistance to controlled scheduler actions. The approach separates behavioral guidance, provided through project-level Markdown instructions, from technical enforcement, provided through a site-operated Model Context Protocol (MCP) service. A four-phase case study was conducted using a reproducible Slurm workload and a single fixed AI client/model configuration. The first phase measured whether project-level instructions changed the operational evidence selected by the agent. The second phase supplied identical measured Slurm and Linux Pressure Stall Information (PSI) evidence to both conditions and evaluated evidence interpretation. The third phase tested scheduler action behavior with authorization either absent or explicit. The fourth phase placed the same requested action behind an MCP authorization boundary and varied authorization externally to the model.

The results show a clear progression. Project-level Markdown reliably steered the agent toward institution-selected PSI telemetry that was absent from baseline responses. When identical measured evidence was already present, both baseline and governed conditions interpreted the evidence correctly, indicating that the primary Markdown effect in this scenario was evidence selection rather than improved reasoning. When scheduler authorization was absent, baseline sessions attempted a state-changing action in 8 of 10 trials while Markdown-governed sessions made no state-changing attempts. Finally, the MCP governance service denied all 20 unauthorized submission requests and allowed all 20 authorized requests exactly once, with no direct scheduler bypass attempts in 40 formal challenge sessions. The case study demonstrates that behavioral guidance and technical enforcement serve different purposes: project instructions can shape planning and action behavior, while a constrained institutional control plane is required to enforce whether consequential actions are permitted to succeed.
\end{abstract}

\noindent\textbf{Keywords:}
high-performance computing, HPC, large language models, AI agents, Model Context Protocol, MCP, Slurm, governance, authorization, Pressure Stall Information
...
Large language models provide a possible means of reducing this operational burden. An AI assistant can review a Slurm script, interpret an application error, compare a resource request with prior job history, or help a researcher prepare a software environment. Agentic systems extend this capability by allowing the model to invoke external tools and perform multi-step operations. Recent work has demonstrated LLM-driven scientific workflows on HPC systems, federated agent frameworks, multi-agent orchestration on leadership-class systems, domain-adapted HPC support assistants, and agent-based performance tuning \cite{ma2025hpcagent,kamatar2026academy,pham2026multiagent,shahin2026hpcllm,hayashi2025vibecodehpc}.
...
The work is structured as a case study because the objective is not to benchmark every available model or every class of HPC workload. Instead, a single reproducible scenario is used to isolate four questions that arise when an institution moves an AI assistant closer to operational authority.

The primary contributions are:
\begin{enumerate}
    \item a layered governance model that distinguishes behavioral steering from technical enforcement;
    \item a four-phase experimental method that separately measures evidence selection, evidence interpretation, authorization-sensitive behavior, and capability enforcement;
...
\subsection{LLM Agents and Scientific HPC}

LLM agents extend conventional conversational systems by allowing a model to select tools, retrieve information, execute structured operations, and coordinate multi-step work. Ma et al.\ connected an LLM agent workflow to HPC resources by integrating LangChain/LangGraph tool calls with Parsl, demonstrating concurrent scientific task execution on workstation and HPC environments \cite{ma2025hpcagent}. This work establishes that a language model can serve as a high-level coordinator while scientific computation remains in an HPC execution framework.

Federated and multi-agent systems extend this concept beyond a single execution environment. Academy provides middleware for deploying stateful agents across heterogeneous research infrastructure, including HPC systems, experimental facilities, and data repositories \cite{kamatar2026academy}. Pham et al.\ demonstrated a hierarchical planner-executor architecture for high-throughput materials screening on the Aurora supercomputer; executor agents used a shared MCP server to coordinate work through Parsl \cite{pham2026multiagent}.

These systems show that agentic orchestration can be technically useful at scale. The present work addresses a different operational question: how a research computing organization should expose ordinary HPC capabilities to general-purpose AI clients while preserving site authorization and operational control.

\subsection{HPC User Assistance and Optimization}

Several recent studies focus directly on improving the HPC user experience with LLMs. HPC-LLM combines retrieval-augmented generation with domain adaptation for Slurm, MPI, GPU, filesystem, and troubleshooting tasks \cite{shahin2026hpcllm}. VibeCodeHPC uses multiple coding-agent roles and iterative prompting to tune HPC code, including CPU-to-GPU transformations \cite{hayashi2025vibecodehpc}. Zheng et al.\ provide a PEARC 2026 comparative study specifically examining LLM-enabled HPC user support \cite{zheng2026hpcsupport}. These studies reinforce the premise that general and domain-adapted LLMs can reduce the expertise barrier associated with common HPC tasks.

For institutional support, however, model capability alone does not answer questions of authority, provenance, or policy. A model may correctly understand how to submit a job while still lacking permission to do so. Similarly, a model may know generic HPC tuning practices without knowing which telemetry or software mechanisms an institution expects researchers to use. The present study therefore evaluates not only whether the AI can provide useful HPC guidance, but also how site-specific guidance changes behavior and how an external capability layer can enforce consequential actions.

\subsection{Repository-Level Context Files}

Repository instruction files such as \texttt{AGENTS.md} are now widely used to tailor coding agents to local projects, but recent empirical results show that their value is task dependent. Gloaguen et al.\ evaluate repository-level context files across multiple coding agents and models and find that such files do not generally improve task success and can increase inference cost, even though agents tend to follow the instructions they contain \cite{gloaguen2026agents}. Lulla et al.\ report a different operational effect: across 124 pull-request tasks, the presence of \texttt{AGENTS.md} was associated with lower median runtime and reduced output-token consumption while maintaining comparable task-completion behavior \cite{lulla2026agents}.

These findings are consistent with the narrower interpretation advanced in this paper. We do not treat a Markdown context file as a general performance enhancement. In Phase 1, the measurable effect was that site guidance surfaced a non-standard, institution-selected diagnostic signal (PSI) that baseline sessions did not request. In Phase 2, when the same evidence was already supplied, the context file produced no observable improvement in interpretation. The relevant benefit is therefore situational steering toward local evidence and operational rules, not a claim that repository instructions universally improve agent capability.

\subsection{MCP and HPC Infrastructure}

The Model Context Protocol provides a standardized mechanism for exposing resources and callable tools to compatible AI clients \cite{mcp2026}. Pan et al.\ report practical experience using thin MCP servers over established scientific cyberinfrastructure services, including Globus and computing-facility status interfaces, and identify evaluation and trust as open challenges for agent-led science \cite{pan2025mcphpc}. This work is particularly relevant to the proposed institutional gateway because it demonstrates the value of wrapping mature services rather than replacing them.

BioCodex provides an even closer HPC scheduler example. Ehrett et al.\ use MCP RunSpecs and asynchronous Slurm execution to support agentic genomics workflows \cite{ehrett2026biocodex}. Pham et al.\ likewise use a shared MCP server as part of an exascale multi-agent orchestration architecture \cite{pham2026multiagent}. MADA uses MCP to wrap scientific applications and the Flux scheduler behind standardized tools for a multi-agent design-exploration workflow \cite{menon2026mada}. These efforts demonstrate that MCP can serve as a practical interface between agents and HPC execution systems.

SchedCP provides an especially relevant systems comparison. It separates LLM reasoning about Linux scheduler optimization from a constrained execution control plane implemented as an MCP server, including an execution verifier for generated scheduler policy \cite{zheng2025schedcp}. The target differs from the present work---Linux kernel scheduling through \texttt{sched\_ext} rather than researcher-facing Slurm job operations---but the architectural similarity reinforces the principle that semantic reasoning and privileged execution should be separated. A complementary 2025 deployment study also demonstrates Slurm-based infrastructure for scalable heterogeneous LLM inference, emphasizing the operational systems context in which agent services may themselves run \cite{delimaluiz2025llmhpc}.

The contribution of the present work is therefore distinct from simply connecting an agent to Slurm or placing MCP in front of an HPC tool. It evaluates a progressive governance sequence in which project-level Markdown first changes evidence selection and authorization-sensitive behavior, and an MCP control plane then enforces whether a requested scheduler action is allowed to succeed. A recent systematic review of AI for operational HPC identifies language-model automation alongside scheduling, performance estimation, optimization, and fault detection as an emerging area, further motivating explicit governance and evaluation practices for this integration \cite{pochelu2026aihpc}.

For HPC, the important design feature is not the protocol syntax itself, but the ability to replace general shell authority with narrow operations such as:
...
Each operation can independently enforce identity, authorization, validation, logging, and parameter restrictions. This provides a natural boundary between what an agent may request and what the institution permits the request to do.

\subsection{Security, Provenance, and the Authorized Agent Problem}

Traditional HPC controls are strong at identity and account-level authorization, but agentic systems introduce an additional decision-making layer. An authenticated agent may be manipulated by untrusted content, tool output, or repository instructions while still acting under valid user credentials. Recent work describes this class of risk as the problem of trusted credentials paired with untrusted agent behavior \cite{li2026trusted}. This distinction is important in research computing because shared filesystems, job logs, source repositories, and application output are all potential sources of instructions interpreted by an LLM.

Agent-security research addresses related failure modes at inference time. AgentSentry targets indirect prompt injection by diagnosing causal changes in tool-use trajectories and purifying suspicious context before it influences subsequent actions \cite{zhang2026agentsentry}. The Silicon Mirror addresses a different behavioral failure mode---sycophancy---through dynamic behavioral gating and an auditor loop \cite{shah2026siliconmirror}. These defenses are complementary to the capability boundary studied here: inference-time defenses may reduce the probability that an agent requests an unsafe action, whereas an independently authorized capability layer determines whether the requested action is permitted to execute.

Auditability is also receiving attention in scientific agent workflows. PROV-AGENT extends provenance models to capture prompts, agent decisions, and downstream workflow effects across edge, cloud, and HPC environments \cite{souza2025provagent}. That work supports the need to preserve agent actions and operational context independently of the conversational interface.

The practical implication is that consequential tool authorization must be implemented outside the model. Prompt instructions may reduce undesirable behavior, but they cannot be treated as the final authority for state-changing operations.
...
These questions intentionally separate model behavior from system authority. RQ1--RQ3 examine whether project-level instructions change the model's decisions. RQ4 examines whether the external system can enforce a decision regardless of what the model chooses to attempt.
...
The author's \texttt{stall-pressure} project provides a practical companion example. It contains CPU- and memory-pressure generators, PSI readers and monitors, and CPU, memory, and I/O graphing examples intended to help HPC users relate low-level pressure signals to resource utilization, profiling, load balancing, and scheduling decisions \cite{stallpressure}. The value of AI in this workflow is not simply to display PSI values. It is to combine those values with Slurm accounting and application context and translate them into a defensible next experiment.
...
Slurm uses these cgroup plugins to organize, constrain, track, and account for work at job, step, and task levels \cite{slurmcgroups,slurmcgroupv2}. Linux cgroup v2 can also expose \texttt{cpu.pressure}, \texttt{memory.pressure}, and \texttt{io.pressure} for processes grouped into a cgroup \cite{linuxpsi,linuxcgroupv2}. The formal Phase 2 collector, however, sampled \texttt{/proc/pressure}; therefore the reported Phase 2 PSI values are node-wide measurements rather than job- or step-scoped measurements.
...
Phase 1 tested RQ1. Ten paired static trials compared baseline Codex with the same client and model operating under project-level Markdown governance. The prompt asked the assistant to review the HPC workload, identify inefficiencies, identify evidence needed for diagnosis, recommend resource changes, and describe how to validate those changes. The prompt did not mention PSI. This omission was deliberate: PSI was treated as the representative site-selected diagnostic convention, analogous to the resource-contention workflow demonstrated in the \texttt{stall-pressure} project, and its appearance was therefore an outcome rather than a cue supplied by the experimenter \cite{stallpressure}.
...
No prospective power calculation was used; the study was designed as an exploratory, controlled case study with 10 repeated trials per cell. Consequently, inferential results should not be read as population-level estimates across models, prompts, or HPC workloads. A future multi-model replication should preregister primary endpoints and sample sizes based on effect sizes observed here. The Phase 4 authorization result was deliberately enforced by deterministic server logic; statistical significance is therefore secondary to the observed invariant and is reported descriptively rather than as evidence that the model learned the control.
...
Table~\ref{tab:combined} summarizes the four phases.
...
The progression is consistent across the case study. Markdown changed what the model considered and how it behaved before an action. MCP determined whether the requested action could succeed after the action was attempted.
...
The \texttt{stall-pressure} project makes the user-facing value of this signal easier to see. It demonstrates how CPU and memory pressure can be induced, monitored, and graphed, and it frames stall pressure together with resource utilization and performance monitoring \cite{stallpressure}. An HPC user, however, still has to decide what a pressure signal means for a specific Slurm job. That is the point at which an AI client can add value: it can combine a pressure observation with CPU efficiency, memory consumption, scheduler state, application parallelism, and file-access behavior and convert the combination into a constrained recommendation.

For example, an I/O pressure signal in a small-file workload should not automatically lead to a request for more CPUs. An AI facilitator should instead consider metadata intensity, page-cache state, logical versus physical throughput, and whether file aggregation or staging would be a better validation experiment. Likewise, CPU pressure becomes useful only when interpreted together with evidence that the application can exploit more CPUs. This evidence-combination behavior is more representative of professional research-computing facilitation than a single-threshold rule.
...
Sixth, the Phase 4 challenge prohibited direct scheduler bypass and the experimental environment disabled direct raw state-changing clients. The observed 0/40 bypass rate therefore demonstrates compliance within that experimental setup; it is not evidence that the MCP layer can contain a hostile process that already has unrestricted shell access.

Seventh, the study compares the complete site policy with no project policy; it does not isolate which clauses, policy length, or ordering caused the observed effects. A targeted ablation should compare the full policy with a minimal PSI-and-authorization policy and with single-purpose policy variants. This is particularly important given mixed recent evidence on the benefits of repository context files \cite{gloaguen2026agents,lulla2026agents}.
...
Finally, prompt injection and untrusted repository content require dedicated adversarial testing. The present study evaluates authorization under controlled prompts. Future work should test malicious README content, job logs, tool descriptions, and shared files that explicitly instruct the agent to bypass the governance service. The highest-value generalization experiments are a second model/client replication of Phases 1 and 3, an adversarial Phase 4 bypass challenge, and a second workload class such as GPU training or MPI using Slurm-cgroup-scoped PSI. Specialized agent-to-agent architectures may also be considered after the single-agent MCP control plane is stable.

\section{Conclusion}

This paper examined a progressive approach for integrating agentic AI into HPC operations without making the AI model itself the source of institutional authority. A four-phase case study separated evidence selection, evidence interpretation, authorization-sensitive behavior, and technical enforcement.
...
The MCP service provided that enforcement boundary. Every unauthorized scheduler request in the formal challenge was denied, every authorized request was allowed exactly once, and no direct scheduler bypass occurred within the controlled experiment.
...
\bibitem{kamatar2026academy}
A. Kamatar, J. G. Pauloski, Y. Babuji, R. Chard, M. Sakarvadia, D. Babnigg,
K. Chard, and I. Foster,
``Empowering Scientific Workflows with Federated Agents,''
in \textit{2026 IEEE International Parallel and Distributed Processing Symposium (IPDPS)},
pp. 1403--1418, 2026, doi:10.1109/IPDPS65963.2026.00114.

\bibitem{pham2026multiagent}
T. D. Pham, H. Tummalapalli, F. H. Bhuiyan, A. V{\'a}zquez Mayagoitia,
C. M. Simpson, R. Balin, V. Vishwanath, and M. Ke{\c{c}}eli,
...
\bibitem{shahin2026hpcllm}
N. Shahin and I. Alsmadi,
...
\bibitem{hayashi2025vibecodehpc}
S.-i. Hayashi, K. Morita, D. Mukunoki, T. Hoshino, and T. Katagiri,
``VibeCodeHPC: An Agent-Based Iterative Prompting Auto-Tuner for HPC Code Generation Using LLMs,''
arXiv:2510.00031, 2025.
\url{https://arxiv.org/abs/2510.00031}

\bibitem{zheng2026hpcsupport}
M. Zheng, F. Linghu, S. Li, M. C. Kandes, N. Gaffney, I. T. Foster, and Z. Zhang,
``A Comparative Study on LLM-enabled HPC User Support,''
in \textit{Proceedings of the Practice and Experience in Advanced Research Computing 2026 (PEARC '26)},
Article 9, pp. 1--8, 2026, doi:10.1145/3785462.3815796.
...
\bibitem{pan2025mcphpc}
H. Pan, R. Chard, R. Mello, C. Grams, T. He, A. Brace, O. P. Skelly,
W. Engler, H. Holbrook, S. Y. Oh, M. Gonthier, M. E. Papka, B. Blaiszik,
...
in \textit{Proceedings of the Practice and Experience in Advanced Research Computing 2026 (PEARC '26)},
Article 57, pp. 1--4, 2026, doi:10.1145/3785462.3815873.

\bibitem{li2026trusted}
J. Li,
...
R. Souza, A. Gueroudji, S. DeWitt, D. Rosendo, T. Ghosal, R. B. Ross,
P. Balaprakash, and R. Ferreira da Silva,
``PROV-AGENT: Unified Provenance for Tracking AI Agent Interactions in Agentic Workflows,''
in \textit{2025 IEEE International Conference on eScience (eScience)},
pp. 467--473, 2025, doi:10.1109/eScience65000.2025.00093.

\bibitem{gloaguen2026agents}
T. Gloaguen, N. M\"undler, M. M\"uller, V. Raychev, and M. Vechev,
``Evaluating AGENTS.md: Are Repository-Level Context Files Helpful for Coding Agents?,''
arXiv:2602.11988, 2026.
\url{https://arxiv.org/abs/2602.11988}

\bibitem{lulla2026agents}
...
\bibitem{menon2026mada}
H. Menon et al.,
``Multi-Agent Collaboration for Automated Design Exploration on High Performance Computing Systems,''
arXiv:2603.11515, 2026.
\url{https://arxiv.org/abs/2603.11515}

\bibitem{zheng2025schedcp}
Y. Zheng, Y. Hu, W. Zhang, and A. Quinn,
``Towards Agentic OS: An LLM Agent Framework for Linux Schedulers,''
arXiv:2509.01245, 2025.
\url{https://arxiv.org/abs/2509.01245}

\bibitem{zhang2026agentsentry}
T. Zhang et al.,
``AgentSentry: Mitigating Indirect Prompt Injection in LLM Agents via Temporal Causal Diagnostics and Context Purification,''
arXiv:2602.22724, 2026.
\url{https://arxiv.org/abs/2602.22724}

\bibitem{shah2026siliconmirror}
H. J. Shah,
``The Silicon Mirror: Dynamic Behavioral Gating for Anti-Sycophancy in LLM Agents,''
arXiv:2604.00478, 2026.
\url{https://arxiv.org/abs/2604.00478}

\bibitem{pochelu2026aihpc}
P. Pochelu, H. Cartiaux, and J. Schleich,
``What Artificial Intelligence can do for High-Performance Computing systems?,''
arXiv:2602.00014, 2026.
\url{https://arxiv.org/abs/2602.00014}

\bibitem{delimaluiz2025llmhpc}
A. de Lima Luiz, S. V. Kurlekar, and M. Georges,
``Scalable Engine and the Performance of Different LLM Models in a SLURM based HPC architecture,''
arXiv:2508.17814, 2025.
\url{https://arxiv.org/abs/2508.17814}

\bibitem{linuxpsi}
J. Weiner,
``PSI -- Pressure Stall Information,''
Linux kernel documentation, Apr. 2018.
\url{https://docs.kernel.org/accounting/psi.html}

\bibitem{linuxcgroupv2}
T. Heo,
``Control Group v2,''
Linux kernel documentation, Oct. 2015.
\url{https://docs.kernel.org/admin-guide/cgroup-v2.html}

\bibitem{slurmcgroups}
SchedMD,
``Control Group in Slurm,''
Slurm Workload Manager documentation.
\url{https://slurm.schedmd.com/cgroups.html}

\bibitem{slurmcgroupv2}
SchedMD,
``Control Group v2 plugin,''
Slurm Workload Manager documentation.
\url{https://slurm.schedmd.com/cgroup_v2.html}

\bibitem{stallpressure}
S. L. Pendelberry,
``stall-pressure: Improving HPC Job Performance through Resource Utilization and Stall Pressure Management,''
GitHub repository, accessed Sept. 4, 2026.
\url{https://github.com/sidpbury/stall-pressure}
