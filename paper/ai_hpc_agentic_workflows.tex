...
BioCodex provides an even closer HPC scheduler example. Ehrett et al.\ use MCP RunSpecs and asynchronous Slurm execution to support agentic genomics workflows \cite{ehrett2026biocodex}. Pham et al.\ likewise use a shared MCP server as part of an exascale multi-agent orchestration architecture \cite{pham2026multiagent}. These efforts demonstrate that MCP can serve as a practical interface between agents and HPC execution systems.

The contribution of the present work is distinct from simply connecting an agent to Slurm. It evaluates a progressive governance sequence in which project-level Markdown first changes evidence selection and authorization-sensitive behavior, and an MCP control plane then enforces whether a requested scheduler action is allowed to succeed.
...
The institution-governed layer exposes narrow MCP tools. Read-only tools can be introduced first because they provide site context without changing scheduler or storage state. State-changing tools require stronger controls.

The Phase 4 prototype provided:
\begin{lstlisting}
...
No generic shell tool was exposed. The \texttt{submit\_job} tool independently evaluated an external authorization state. When authorization was absent, the tool returned a denial. When authorization was granted, the experiment returned a simulated validation result. Production Slurm was intentionally unreachable from the experimental state-changing path.
...
Paired exact tests were used where the design produced paired baseline and Markdown observations. For the Phase 1 PSI composite, all 10 pairs differed in the same direction. For the Phase 3 authorization-absent comparison, eight paired trials differed in the same direction and two tied. The Phase 4 authorization result was deliberately enforced by deterministic server logic; statistical significance is therefore secondary to the observed invariant and is reported descriptively rather than as evidence that the model learned the control.
...
Using the original 17-item rubric, the mean baseline score was 13.0/17 (76.5\%) and the Markdown-governed mean was 15.9/17 (93.5\%). Because the three PSI rows were correlated, the conservative composite analysis produced 13/15 (86.7\%) for baseline and 14/15 (93.3\%) for Markdown, a 6.7 percentage-point difference. At least one PSI domain was mentioned in 10/10 governed trials and 0/10 baseline trials (exact paired $p=0.001953$).

The result does not show that Markdown made the model generally better at HPC diagnosis. It shows that the project policy reliably changed which institution-selected evidence entered the model's proposed diagnostic process.
...
All 40 sessions materially right-sized the original 20 GiB / 20 minute request while preserving the one-CPU/no-GPU geometry and correctness gate. The final memory requests ranged from 64 MiB to 256 MiB, and walltime requests were one or two minutes. This variation reflects model discretion within the validation scenario and did not affect the authorization outcome.
...
The experiment retained prompts, policies, trial outputs, project diffs, tool-call logs, scorecards, and formal result summaries. This level of artifact preservation is important because LLM outputs are probabilistic and client implementations change over time.

For a production service, a similar approach should apply to state-changing operations. The audit record should contain the authenticated user, tool, parameters, timestamp, authorization result, resulting scheduler identifier where applicable, and enough context to reconstruct the action. The natural-language conversation may have different retention requirements, but the operational action record should be independent of the conversation transcript.
...
Finally, prompt injection and untrusted repository content require dedicated adversarial testing. The present study evaluates authorization under controlled prompts. Future work should test malicious README content, job logs, tool descriptions, and shared files to determine whether the external capability boundary continues to hold when the agent is actively manipulated. Specialized agent-to-agent architectures may also be considered after the single-agent MCP control plane is stable.
